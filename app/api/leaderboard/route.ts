...
// In-memory cache for leaderboard data (30 second TTL)
let cachedLeaderboard: Array<{
  address: string;
  realizedPnl: string;
  totalVolume: string;
  totalTrades: number;
...
async function getLeaderboardData() {
  ...
  // Fetch and calculate fresh data
  ...
  // Update cache
  cachedLeaderboard = tradersWithRank;
  cacheTimestamp = now;
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Re-sort if needed (cache is sorted by P&L)
    if (sortBy === "volume") {
